...
class History3Month:
    ...
    def add_data(self, industry_id, date, open, high, low, close, volume, dividends, stock_splits):
        self.cursor.execute("""
            INSERT INTO history_3month (industry_id, Date, Open, High, Low, Close, Volume, Dividends, StockSplits)
    ...
    def delete_data(self, id):
        return self.cursor.execute("DELETE FROM history_3month WHERE id = %s", (id,))

    # Dateに重複がない場合のみ、データの追加
    def add_data_if_not_exists(self, industry_id, date, open, high, low, close, volume, dividends, stock_splits):
        if self.cursor.fetchone("SELECT COUNT(*) FROM history_3month WHERE Date = %s", (date,)) == 0:
    ...
    def get_data_by_id(self, id):
        return self.cursor.fetchone("SELECT * FROM history_3month WHERE id = %s", (id,))

    # Dateで絞り込み、industry_idからデータを抜き出し
    def get_data_by_date_and_industry_id(self,date, industry_id):
        return self.cursor.fetchall("SELECT * FROM history_3month WHERE Date = %s AND industry_id = %s", (date, industry_id))

    # Dateで絞り込んだデータを抜き出し
    def get_data_by_date(self, date):
        return self.cursor.fetchall("SELECT * FROM history_3month WHERE Date = %s", (date,))
